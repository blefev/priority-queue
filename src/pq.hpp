...
template <class T> void PQ<T>::enqueue(T &data, int priority) {
...
template <class T> void PQ<T>::dequeue(void) {
  if (!this->head)
    return;
...
template <class T> T PQ<T>::get_front() {
  if (this->head) {
...
template <class T> bool PQ<T>::isEmpty() { return this->head == NULL; }

template <class T> void PQ<T>::clear(void) {
  while (this->head) {
...
template <class T> int PQ<T>::get_count(void) {
